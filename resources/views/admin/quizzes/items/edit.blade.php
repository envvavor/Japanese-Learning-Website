        ...
        <form action="{{ route('admin.quizzes.items.update', [$quiz, $item]) }}" method="POST" id="itemForm" class="space-y-8">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <button type="button" onclick="selectType('multiple_choice')" data-type="multiple_choice" class="type-btn p-4 rounded-xl border-2 transition-all group">
                        <i class="fas fa-list-ul text-indigo-500 text-xl mb-2"></i>
                        <p class="font-bold text-gray-800 dark:text-gray-200">Pilihan Ganda</p>
                    </button>
                    <button type="button" onclick="selectType('drawing')" data-type="drawing" class="type-btn p-4 rounded-xl border-2 transition-all group">
                        <i class="fas fa-pencil-alt text-emerald-500 text-xl mb-2"></i>
                        <p class="font-bold text-gray-800 dark:text-gray-200">Menggambar</p>
                    </button>
                    <button type="button" onclick="selectType('listening')" data-type="listening" class="type-btn p-4 rounded-xl border-2 transition-all group">
            ...
            <div>
                ...
                <textarea name="question_text" id="question_text" rows="2" required
                          class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all">{{ old('question_text', $item->question_text) }}</textarea>
            </div>
            ...
            <div id="mc-section" class="space-y-4 p-5 bg-indigo-50 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-indigo-900/50">
                <p class="text-sm font-bold text-indigo-700 dark:text-indigo-400 mb-2">Pilihan Jawaban</p>
                <div class="space-y-3">
                    @for($i = 0; $i < 4; $i++)
                    @php $optValue = $optionsArray[$i] ?? ''; @endphp
                    <div class="flex items-center gap-3">
                        <input type="radio" name="correct_answer" value="{{ $optValue }}" class="mc-radio w-5 h-5 text-indigo-600 border-gray-300 dark:border-gray-600 focus:ring-indigo-500 bg-white dark:bg-gray-900" {{ $optValue === $item->correct_answer && $optValue !== '' ? 'checked' : '' }}>
                        <input type="text" name="options[]" value="{{ $optValue }}" placeholder="Opsi {{ $i + 1 }}"
                               class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
            ...
            <div id="drawing-section" class="space-y-4 hidden p-5 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-900/50">
                <p class="text-sm font-bold text-emerald-700 dark:text-emerald-400 mb-2">Target Karakter (Kanji)</p>
                <div class="relative">
                    <div class="flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-emerald-500">
                        ...
                        <input type="text" id="kanji_search" placeholder="Ganti karakter... (Ketik untuk mencari)" class="w-full px-3 py-3 bg-transparent border-none focus:ring-0 text-sm dark:text-gray-200">
                    </div>
                ...
                <div id="selected_kanji_display" class="{{ $item->kanji ? '' : 'hidden' }} mt-4 flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-xl border-2 border-emerald-200 dark:border-emerald-800">
                    <div class="flex items-center gap-4">
                        <span id="selected_kanji_char" class="text-4xl font-black text-emerald-600 dark:text-emerald-400">{{ $item->kanji->character ?? '' }}</span>
                        <div>
                            <p id="selected_kanji_meaning" class="text-sm font-bold text-gray-800 dark:text-gray-100">{{ $item->kanji->meaning ?? '' }}</p>
                            <p id="selected_kanji_cat" class="text-xs text-gray-500">{{ $item->kanji ? ucfirst($item->kanji->category) : '' }}</p>
                        </div>
                    </div>
            ...
            <div id="listening-section" class="hidden space-y-4 p-5 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-900/50">
                <p class="text-sm font-bold text-amber-700 dark:text-amber-400 mb-2">Setup Audio</p>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-2 flex gap-2">
                        <input type="text" id="tts_text" placeholder="Masukkan tulisan Jepang..." class="flex-1 px-4 py-2.5 rounded-xl border border-amber-200 dark:border-amber-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">
                        <button type="button" onclick="regenerateAudio()" id="regen_btn" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold rounded-xl shadow transition-all flex items-center gap-2">
                            <i class="fas fa-redo" id="regen_icon"></i> Re-Gen
                        </button>
                    </div>
                </div>

                {{-- Audio Preview --}}
                <div id="audio_preview_wrap" class="{{ $item->audio_url ? '' : 'hidden' }} mt-3">
                    <div class="flex items-center gap-3 p-3 bg-white dark:bg-gray-900 rounded-xl border border-amber-200 dark:border-amber-700">
                        ...
                        <audio id="audio_preview" controls class="flex-1 h-10" src="{{ $item->audio_url }}"></audio>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    @php $optValue = $optionsArray[$i] ?? ''; @endphp
                    <div class="flex items-center gap-3">
                        <input type="radio" name="correct_answer" value="{{ $optValue }}" class="listening-radio w-5 h-5 text-amber-500 border-gray-300 dark:border-gray-600 focus:ring-amber-500 bg-white dark:bg-gray-900" {{ $optValue === $item->correct_answer && $optValue !== '' ? 'checked' : '' }}>
                        <input type="text" name="options[]" value="{{ $optValue }}" placeholder="Opsi {{ $i + 1 }}"
                               class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all"
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                <a href="{{ route('admin.quizzes.show', $quiz) }}" class="flex-1 py-3 text-center text-sm font-semibold text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-all">Batal</a>
...
<script>
    ...
    function selectType(type) {
        ...
        document.querySelectorAll('.type-btn').forEach(btn => {
            ...
            if (t === type) {
                ...
                btn.className = 'type-btn p-4 rounded-xl border-2 border-slate-200 dark:border-gray-600 text-left hover:border-gray-400 dark:hover:border-gray-500 transition-all group';
            }
        });

        // Toggle Section Display & MENCEGAH BUG DUPLIKAT SUBMIT
        const config = {
        ...
        for (const [key, id] of Object.entries(config)) {
            ...
            if (key === type) {
                ...
                // Aktifkan kembali input agar dikirim ke server
                section.querySelectorAll('input, select, textarea').forEach(el => el.disabled = false);
            } else {
                section.style.display = 'none';
                // Matikan input agar tidak bentrok dengan section lain saat submit
        ...
        try {
            ...
            if (!data.length) { 
                ...
                box.innerHTML = data.map(k => `
                    <button type="button" onclick='selectKanji(${JSON.stringify(k)})' class="w-full text-left px-4 py-3 hover:bg-indigo-50 dark:hover:bg-gray-700 flex items-center gap-3 border-b border-gray-100 dark:border-gray-700 transition-colors">
    ...
    // Inisialisasi tampilan pertama kali saat halaman dimuat
